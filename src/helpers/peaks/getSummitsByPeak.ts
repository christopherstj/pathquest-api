...
const getSummitsByPeak = async (peakId: string, userId: string) => {
    ...
    const rows = (
        await db.query(
            `
            SELECT ap.id, ap.timestamp, ap.activity_id, ap.notes, ap.is_public, ap.temperature, ap.precipitation, ap.weather_code, ap.cloud_cover, ap.wind_speed, ap.wind_direction, ap.humidity, ap.difficulty, ap.experience_rating, ap.timezone
            FROM (
